...
class ChargingService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        if (intent?.action == "ACTION_PLUG_STOP_SERVICE") {
            stopForeground(true)
            stopSelf()
        }
        return START_STICKY
    ...
    override fun onCreate() {
        ...
        val filter = IntentFilter().apply {
            addAction("ACTION_PLUG_STOP_SERVICE")
        }
        ...
        // Register the receiver for charging state changes
    ...
    private fun startForegroundServiceCompat() {
        try {
            ...
            val stopServiceIntent = Intent(this, ChargingService::class.java).apply {
                action = "ACTION_PLUG_STOP_SERVICE" // Define a custom action for stopping the service
            }
            ...
            val notificationBuilder = NotificationCompat.Builder(this, channelId)
                .setContentTitle("Unplug Service Running")
    ...
    override fun onDestroy() {
        val intent = Intent("charging_state_changed")
        intent.putExtra("isCharging", false) // Update value based on your logic
        LocalBroadcastManager.getInstance(this).sendBroadcast(intent)

        preferencesManager.isChargingRunningSet = false
    ...
    private val chargingReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            ...
            if (action == Intent.ACTION_POWER_DISCONNECTED && !isPlaying) {
                serviceScope.launch {
                    playAlarmActions()
                }
            }
    ...
    private fun playAlarmActions() {
        ...
        isPlaying = true
        preferencesManager.isChargingRunningSet = true
    ...
    companion object {
        ...
        val channelId = "unplug_service_channel"
        val channelName = "UnPlug Service Channel"
    }
}