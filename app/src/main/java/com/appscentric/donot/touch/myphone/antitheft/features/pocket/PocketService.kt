...
class PocketService : Service(), SensorEventListener {
    ...
    private fun upDateUiState() {
        val intent = Intent("pocket_state_changed")
        intent.putExtra("isPocketModeRunning", false) // Update value based on your logic
    ...
    private fun startForegroundServiceCompat() {
        try {
            ...
            val stopServiceIntent = Intent(this, PocketService::class.java).apply {
            ...
            val notificationBuilder = NotificationCompat.Builder(this, channelId)
                .setContentTitle("Pocket Mode Service Running")
                .setContentText(getString(R.string.tap_to_deactivate))
                .setSmallIcon(R.mipmap.ic_launcher_round).setContentIntent(launchAppPendingIntent)
    ...
    private fun showFlashlightNotAvailableMessage() {
        Toast.makeText(
            this, "Flashlight is not available on this device", Toast.LENGTH_SHORT
        ).show()
    ...
    override fun onSensorChanged(event: SensorEvent) {
        if (event.sensor.type == Sensor.TYPE_ACCELEROMETER) {
            if (!isPlaying && !preferencesManager.pocketMode && playOnce) {
                playOnce = false
                serviceScope.launch {
                    playAlarmActions()
                }
            ...
            if (event.sensor.type == Sensor.TYPE_PROXIMITY) {
                if (event.values[0] < proximitySensor!!.maximumRange) {
                    ...
                    if (isInPocket) {
                        ...
                        sensorManager.registerListener(
                            orientationListener, accelerometer, SensorManager.SENSOR_DELAY_UI
                        )
                        sensorManager.registerListener(
                            orientationListener, magnetometer, SensorManager.SENSOR_DELAY_UI
                        )
                    }
                    isInPocket = false
                }
...
//                val serviceIntent =
//                    Intent(this@PocketService, FullScreenDialogService::class.java)
...
//        }
        ...
        if (preferencesManager.isPinSetup()) {
            val serviceIntent = Intent(this@PocketService, PinDialogService::class.java)
            serviceIntent.putExtra("volumeDurationMillis", volumeDurationMillis)
    ...
    companion object {
        ...
        private const val channelId = "pocket_service_channel"
        private const val channelName = "Pocket Service Channel"
    }
}